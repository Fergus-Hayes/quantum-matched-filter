...
\begin{equation}
   \label{InverQFT}
   \hat{U}^{-1}_{QFT}\sum_{a=0}^{2^N-1}e^{i2\theta_t a}\ket{a}\otimes\ket{s_t}=\frac{1}{2^{N}}\sum_{a=0}^{2^N-1} \sum_{b=0}^{2^N-1} e^{i2\pi a(\frac{\theta_t}{\pi}-\frac{b}{2^N})}\ket{b}\otimes\ket{s_t}.%this part can elaborate in thesis according to steve's 7.38
\end{equation}
...
\begin{algorithmic}[1]
\State $\textit{n} \gets \text{number of }\textit{precision digits}$
...
\State{$ \ket{i}\ket{T_i}\gets \ket{0}^n\ket{i}\ket{T_i}$}
\State{$ \ket{0}^n\ket{i}\ket{T_i}\gets \frac{1}{2^{n/2}}(\ket{0}+\ket{1})^n\otimes\ket{i}\ket{T_i}$}
...
\ForAll{$a<2^n$}
   \State $j \gets a$
    ...
    \Until{$j==0$}
\EndFor
\State$\frac{1}{2^{n/2}}(\ket{0}+\ket{1})^n\otimes\ket{i}\ket{T_i} \gets \frac{1}{2^{n/2}}\sum e^{2i\theta_t a}\ket{a}\otimes\ket{i}\ket{T_i} $
\State \emph{Inverse Quantum Fourier Transform}:
    \State$\frac{1}{2^{n/2}}\sum e^{2i\theta_t a}\ket{a}\otimes\ket{i}\ket{T_i}\gets \frac{1}{2^{n}}\sum \sum e^{2\pi a i(\frac{\theta_t }{\pi}-\frac{x}{2^n})}\ket{x}\otimes\ket{i}\ket{T_i}$
\State \emph{Measurement}:
    \State$x-\frac{2^n\theta_t }{\pi}=0 $ %constructive interference across all m
\State \emph{Output}:
    \State$\frac{\theta_t }{\pi}\gets\frac{x}{2^n} $
%$(2k_t+1)=\frac{\pi}{2\theta_t}$ $k_t=\frac{\pi}{4\theta_t}-1/2$
\State $k_t \gets \textbf{Floor}[\frac{2^n}{4x}-\frac{1}{2}]$
\EndProcedure
