...
def main(infiles, outpath, fontsize=28, ticksize=22, figsize=(10,8)):
    ...
    for infile in infiles:
        ...
        probs.append(np.sum(np.abs(np.load(infile))**2,axis=1))
    ...
    for i,prob in enumerate(probs):
        ...
        ax2.bar(np.arange(len(prob)),prob, label=r'$\rho_{\regular{th}}=$'+str(int(SNRs[i])))

    stretch = 70 #len(prob)//9
    ...
    fig.text(.5, .04, xlabel, ha='center', fontsize=fontsize)
    ...
    fig.savefig(outpath+'_'.join(infiles[-1].split('/')[-1].split('_')[:4])+'_snr_'+'_'.join(SNRs.astype(int).astype(str))+'_psi2_b_paper.png')
