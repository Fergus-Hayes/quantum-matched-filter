...
def main(infiles, outpath, noisefile=False, fontsize=28, ticksize=22, figsize=(12,8)):
    ...
    label_match = r'$\rho \geq \rho_{\regular{th}} (= 18)$'
    label_nmatch = r'$\rho < \rho_{\regular{th}} (= 18)$'
    ...
    for i,infile in enumerate([infiles[-1]]):
        ...
        axes[i].annotate(text='', xy=(k_t,1), xytext=(1,1), arrowprops=dict(arrowstyle='<->'))
        axes[i].annotate(text=r'$k_{t}$', xy=(k_t/2,1.005), fontsize=4*fontsize/5)
        ...
        leg = axes[i].legend(fontsize=3*fontsize//4, loc='upper right')
