...
#bash wrapper.sh ${ENVLOC} ${ENV} ${PWD}/QMF_150914.py --Mq ${M} --Pq ${P} --tag ${TAG} --bank bank --SNR-thr ${SNR} --cores ${CORES} --data-file data/signal_32.npy --psd-file data/psd_32.npy --path ${LOC}/${RUN}/
...
echo "arguments = ${ENVLOC} ${ENV} ${PWD}/QMF_150914.py --Mq ${M} --Pq ${P} --tag ${TAG} --bank bank --SNR-thr ${SNR} --cores ${CORES} --data-file data/signal_16s.npy --psd-file data/psd_16s.npy --path ${LOC}/${RUN}/">>"${LOC}/${RUN}/sample.sub"

#${ENVLOC} ${ENV} ${PWD}/QMF_150914.py --Mq ${M} --Pq ${P} --tag ${TAG} --bank bank --SNR-thr ${SNR} --cores ${CORES} --data-file data/signal.npy --psd-file data/psd.npy --path ${LOC}/${RUN}/">>"${LOC}/${RUN}/sample.sub"
