...
echo "arguments = ${ENVLOC} ${ENV} ${PWD}/QMF_150914.py --Mq ${M} --Pq ${P} --tag ${TAG} --bank grid --SNR-thr ${SNR} --cores ${CORES} --data-file data/noise.npy --psd-file data/noise_psd.npy --spinless --path ${LOC}/${RUN}/">>"${LOC}/${RUN}/sample.sub"
